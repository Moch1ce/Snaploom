...
5. `release-qualification` 与 `release-draft` environment 都只允许 `main` deployment。单人仓库允许
   所有者批准自己触发的 deployment，并保留 admin bypass。RC workflow 不使用 `release-signing`，
   不读取 Apple Developer ID 证书或 App Store Connect 公证凭据；工作流只允许从 `main` dispatch，
   所有构建仍显式 checkout 已验证 tag commit。

当前已有的公开 `v0.1.0` prerelease 占用了该 tag，不能删除、移动或替换后复用。首个稳定 RC
必须在合入后使用新的 patch 版本完成 release prep。

## 运行输入与阶段

手动运行 `Stable release candidate draft` 时提供 `version`、完整 `commit`、成功的 `ci_run_id` 和
...
2. Windows 以 `stable-unsigned` 构建 Desktop、Host、installer 与 C SDK DLL，确认它们均未签名，并把
   未知发布者/SmartScreen 风险写入 metadata、Release notes 与所有者审批包。
3. macOS 以 `stable-unsigned` 构建 App/Host/DMG，只使用无发布者身份的 ad hoc 结构签名，不提交
   Apple 公证；验证 metadata、ad hoc 签名、架构、安装和启动，并把 Gatekeeper 风险写入 Release notes
   与所有者审批包。SDK dylib/XCFramework 保持 consumer-resignable。
4. 用同次未签名 Windows DLL 和 macOS dylib 生成双 RID NuGet，并在 Windows/macOS 分别执行
   .NET 8/.NET 10 的 package-only build/run、trim 与 NativeAOT；C/C++、Swift consumer 也直接消费
   同次最终 archive。
5. 保存 Windows/macOS runner image 与工具链版本、平台信任模式、二进制检查和包验证日志；对应源码
   独立构建两次并逐字节比较。
6. 汇总精确 12 个 payload、独立 checksum、`SHA256SUMS`、SBOM/NOTICE 与 `stable-release` manifest，
...
8. 再次从远端 tag ref 解析真实 commit，通过 Release asset API 重新下载全部 26 个权威文件，复验名称、size、GitHub `sha256:` digest、
   manifest、双平台未签名风险证据、许可证边界和 Swift checksum。
9. 上传 `snaploom-vX.Y.Z-issue-33-owner-approval` Actions artifact，其中含 draft URL、tag/commit、manifest
   digest、全部 payload checksums、Windows 未签名/SmartScreen 风险、macOS 无 Developer ID/未公证/
   Gatekeeper 风险与验证日志、builder identity、完整 CI、最终
   consumer 与 hosted 资格证据，并保留无交互桌面/无真机性能证明的限制。

## 失败与所有者风险接受门禁

Windows 未签名校验、macOS stable-unsigned/ad hoc 校验、上传、重下载或消费校验任一步失败都不会公开 Release。若上传中途失败，保留的 partial
