...
正式稳定 Windows 与 macOS 包都使用 `stable-unsigned`，并依靠 checksum 与 GitHub attestation 验证来源。
macOS 包只有 ad hoc 结构签名，没有 Developer ID 身份或 Apple 公证，必须同时披露 Gatekeeper 风险。
该平台信任策略不改变修改版 Host 对公开协议的可用性。
