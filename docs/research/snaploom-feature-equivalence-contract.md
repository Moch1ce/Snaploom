...
- 编辑框按已提交文本与 IME 预编辑串的完整内容实时增长；单次编辑期间不得在候选确认时立即收缩，避免输入法确认后光标沿用失效布局。候选上屏前不得横向滚动隐藏开头，靠近选区边缘时裁限在选区内。内部水平和垂直滚动条必须禁用且不可见，鼠标滚轮不得滚动输入内容；选区不足以容纳完整单行及四边内边距时不得启动文字输入。
