...
状态：**工程事实与复核表已准备；外部律师尚未签署，不得发布首个稳定 SDK。**

对应 Issue：[#33](https://github.com/moch1ce/Snaploom/issues/33)

> 本文是提供给熟悉开源软件许可的律师的事实包和工程核对表，不是法律意见。未填写第 9 节的外部复核记录前，任何人不得将本文描述为“已经法务批准”。
...
律师必须在最终候选构建上核对此表。文件名可由发布设计票调整，但产物边界不变。
...
以下字段必须由真实复核人员填写，不允许 CI、机器人或项目维护者代填：
...
如意见文件含特权或个人信息，仓库只保留复核人认可的公开摘要、不可变文档哈希、日期、结论与受控存档位置；不把特权文件上传到 public repository。
