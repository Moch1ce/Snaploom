...
export function verifyQualificationSet(directory, { version, commit }) {
  ...
  if (
    byPlatform.size !== hostedQualificationPolicy.length ||
    documents.length !== hostedQualificationPolicy.length
  ) {
    throw new Error("qualification set must contain exactly two unique hosted platform evaluations");
  }
  for (const identity of hostedQualificationPolicy) {
    ...
    if (
      ...
      !isDeepStrictEqual(document.limitations, hostedQualificationLimitations)
    ) {
      throw new Error(
  ...
  return hostedQualificationPolicy.map(({ platform }) => byPlatform.get(platform));
}
...
if (process.argv[1] && basename(process.argv[1]) === "verify-qualification-set.mjs") {
  ...
  process.stdout.write(
    `qualified ${hostedQualificationPolicy.map(({ platform }) => platform).join(", ")} on GitHub-hosted runners for ${commit}\n`,
  );
}
